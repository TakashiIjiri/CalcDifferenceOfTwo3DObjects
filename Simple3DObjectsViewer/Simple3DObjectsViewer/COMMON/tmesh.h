...
#pragma omp parallel for
  ...
  float GetMeanDistOfVertsFromGravityCenter() const
  {
    EVec3f gc = getGravityCenter();
    
    float d = 0;
    for (int i = 0; i < m_vSize; ++i) 
      d += t_dist(m_vVerts[i], gc);
    return d / (float)m_vSize;
  }

  float GetDistBetweenFathestTwoVerts() const
  {
    float max_dist = 0;
    ...
    for (int i = 0; i < m_vSize; ++i)
    {
      for (int j = i+1; j < m_vSize; ++j)
        max_dist = std::max( max_dist, t_dist_sq(m_vVerts[i], m_vVerts[j] ) );
      if( i % 10000 == 0 )
        std::cout << i / (double) m_vSize << "done\n" ; 
    }
